<script lang="ts">
  ...
  import {
    adminListHomeImages,
    adminCreateHomeImage,
    adminPatchHomeImage,
    adminDeleteHomeImage,
    adminReorderHomeImages
  } from '$lib/api.admin';
  ...
  async function load() {
    loading = true;
    try {
      const data = await adminListHomeImages();
  ...
  async function upload() {
    ...
    creating = true;
    try {
      await adminCreateHomeImage({ file, title, caption, order, is_active: true });
  ...
  async function toggleActive(it: any) {
    ...
    try {
      await adminPatchHomeImage(id, { is_active: !prev });
    } catch (e: any) {
      updateLocal(id, { is_active: prev });
      toastError(String(e?.message ?? e));
    } finally {
  ...
  const saveFieldDebounced = debounce(async (id: number, patch: any) => {
    ...
    try {
      await adminPatchHomeImage(id, patch);
    } catch (e: any) {
      toastError(String(e?.message ?? e));
      await load();
    } finally {
  ...
  async function remove(it: any) {
    const id = it.id;
    if (!confirm('¿Eliminar esta imagen de inicio?')) return;
    busyDelete = new Set(busyDelete).add(id);
    try {
      await adminDeleteHomeImage(id);
  ...
  async function saveOrders() {
    ...
    try {
      ...
      await adminReorderHomeImages(payload);
...
<section class="mx-auto max-w-7xl px-4 py-6 space-y-6">
  ...
  <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div class="space-y-1">
      ...
      <p class="text-slate-600 text-sm sm:text-base">
        Gestiona el slider principal de la página de inicio: títulos, descripciones, orden y estado.
      </p>
    </div>
    ...
    <div class="flex flex-col items-start sm:items-end gap-1 text-xs text-slate-500">
      <span class="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1">
        ...
        <span>
          {activeImages} activas de {totalImages} imágenes
        </span>
      </span>
      <button
        type="button"
  ...
  <!-- Formulario creación -->
  <div class="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-4 sm:p-5 shadow-sm">
    <div class="flex items-center justify-between gap-3 mb-3">
      <div>
        <h2 class="text-sm font-semibold text-slate-900">Nueva imagen para el slider</h2>
        <p class="text-xs text-slate-500 mt-0.5">
          Sube una imagen grande (ideal 1600×600px) y define el texto que se mostrará sobre el banner.
        </p>
      </div>
    </div>

    <div class="mt-2 grid gap-4 md:grid-cols-5">
      <!-- Selector de archivo + preview -->
      <div class="md:col-span-2 flex flex-col gap-2">
        <p class="text-xs font-medium text-slate-600">Imagen</p>
        <div class="flex items-center gap-3">
          <label class="relative inline-flex cursor-pointer">
            <input type="file" accept="image/*" class="sr-only" on:change={onPick} />
            <span
              class="inline-flex items-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"
            >
          ...
          {#if preview}
            <div class="flex items-center gap-2">
              <img
                src={preview}
                alt="preview"
                class="h-12 w-12 rounded-xl object-cover ring-1 ring-slate-200 shadow-sm"
              />
              <button
                type="button"
        ...
        <p class="text-[11px] text-slate-400">
          Formatos recomendados: JPG o PNG. Peso máximo ~2&nbsp;MB.
        </p>
      </div>
      ...
      <label class="md:col-span-1">
        ...
        <input
          ...
          placeholder="Ej: Diseños a medida"
          bind:value={title}
        />
      </label>
  ...
  <div class="space-y-4">
    ...
    {:else if !items.length}
      <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 px-4 py-10 text-center text-sm text-slate-500">
        Aún no has agregado imágenes al inicio. Sube una imagen con el formulario superior
        para comenzar a armar el slider.
      </div>
    {:else}
          ...
          <div class="group rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition">
            ...
            <div class="p-4 space-y-3">
              ...
              <div class="flex items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                  <input
                    class="w-24 rounded-xl border border-slate-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/70"
                    type="number"
                    bind:value={it.order}
                    title="Orden"
                  />
