...
function _page($$payload, $$props) {
  ...
  $$payload.out.push(`<!--]--></button></div></header> <div class="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-4 sm:p-5 shadow-sm"><div class="flex items-center justify-between gap-3 mb-3"><div><h2 class="text-sm font-semibold text-slate-900">Nueva imagen para el slider</h2> <p class="text-xs text-slate-500 mt-0.5">Sube una imagen grande (ideal 1600×600px) y define el texto que se mostrará sobre el banner.</p></div></div> <div class="mt-2 grid gap-4 md:grid-cols-5"><div class="md:col-span-2 flex flex-col gap-2"><p class="text-xs font-medium text-slate-600">Imagen</p> <div class="flex items-center gap-3"><label class="relative inline-flex cursor-pointer"><input type="file" accept="image/*" class="sr-only"/> <span class="inline-flex items-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none"><path d="M12 5v14m0-14L8 9m4-4 4 4" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"></path></svg> Seleccionar archivo</span></label> `);
  {
    $$payload.out.push("<!--[!-->");
  }
  $$payload.out.push(`<!--]--></div> <p class="text-[11px] text-slate-400">Formatos recomendados: JPG o PNG. Peso máximo ~2 MB.</p></div> <label class="md:col-span-1"><span class="block text-xs text-slate-600 mb-1">Título</span> <input class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/70" placeholder="Ej: Diseños a medida"${attr("value", title)}/></label> <label class="md:col-span-1"><span class="block text-xs text-slate-600 mb-1">Descripción</span> <input class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/70" placeholder="Texto breve bajo el título"${attr("value", caption)}/></label> <label class="md:col-span-1"><span class="block text-xs text-slate-600 mb-1">Orden</span> <input class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/70" type="number" min="0"${attr("value", order)}/> <p class="mt-1 text-[11px] text-slate-400">Menor número → aparece primero.</p></label></div> <div class="mt-4 flex justify-end"><button type="button"${attr("disabled", creating, true)} class="inline-flex items-center gap-2 rounded-xl bg-amber-600 text-white px-4 py-2 text-sm font-medium shadow hover:bg-amber-700 disabled:opacity-60">`);
  {
    $$payload.out.push("<!--[!-->");
  ...
  if (items.length) {
    $$payload.out.push("<!--[-->");
    $$payload.out.push(`<div class="mt-4 flex justify-end"><button type="button" class="inline-flex items-center gap-2 rounded-xl bg-slate-900 text-white px-4 py-2 text-sm font-medium shadow hover:bg-slate-800 disabled:opacity-60"${attr("disabled", busyOrder, true)}>`);
    {
      $$payload.out.push("<!--[!-->");
